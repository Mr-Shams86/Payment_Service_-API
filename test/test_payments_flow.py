...
async def test_confirm_payment(client):
    r, _ = await create_payment(client, order_id=30001, amount=10.0)
    pid = r.json()["id"]
...
async def test_fail_flow(client):
    r, _ = await create_payment(client, order_id=30002, amount=10.0)
    pid = r.json()["id"]
...
async def test_refund_flow(client):
    r, _ = await create_payment(client, order_id=30003, amount=10.0)
    pid = r.json()["id"]
